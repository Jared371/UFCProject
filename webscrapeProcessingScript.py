...
# Define CSV headers
finalHeader = ["Name", "Age", "Wins", "Total", "WinPercentage", "WeightClass", "GenderBool", "ChampBoutBool"]
...
def processRawData():
    ...
    for fight in rawData:
        ...
        processedFight = [None] * 8
        
        # Process Winner
        winner_name = fight[2]
        winner_birthday = fight[6]
        processedFight[0] = winner_name
        processedFight[1] = calculateAge(winner_birthday, event_date_str) if winner_birthday else None
        processedFight[2] = len([f for f in rawData if f[2] == winner_name and datetime.strptime(f[1], "%B %d, %Y") <= event_date])
        processedFight[3] = len([f for f in rawData if (f[2] == winner_name or f[3] == winner_name) and datetime.strptime(f[1], "%B %d, %Y") <= event_date])
        processedFight[4] = float(processedFight[2]) / float(processedFight[3]) if processedFight[3] != 0 else 0
        processedFight[5] = fight[5]
        processedFight[6] = fight[9]
        processedFight[7] = fight[8]
        ...
        processedFight[0] = loser_name
        processedFight[1] = calculateAge(loser_birthday, event_date_str) if loser_birthday else None
        processedFight[2] = len([f for f in rawData if f[2] == loser_name and datetime.strptime(f[1], "%B %d, %Y") <= event_date])
        processedFight[3] = len([f for f in rawData if (f[2] == loser_name or f[3] == loser_name) and datetime.strptime(f[1], "%B %d, %Y") <= event_date])
        processedFight[4] = float(processedFight[2]) / float(processedFight[3]) if processedFight[3] != 0 else 0
        processedFight[5] = fight[5]
        processedFight[6] = fight[9]
        processedFight[7] = fight[8]
...
def main():
    ...
    if rawData:
        file_path = 'C:\\Users\\Jared\\Desktop\\UFCRawData1.csv'
        writeToCSV(rawData, rawHeader, file_path)
    else:
        print("No Data In rawData\n")
    if failData:
        file_path = 'C:\\Users\\Jared\\Desktop\\UFCFailedRawData1.csv'
        writeToCSV(failData, rawHeader, file_path)
    else:
        print("No Data In failData\n")
    if finalData:
        file_path = 'C:\\Users\\Jared\\Desktop\\UFCData1.csv'
        writeToCSV(finalData, finalHeader, file_path)
    else:
        print("No Data In finalData\n")
    if finalFailData:
        file_path = 'C:\\Users\\Jared\\Desktop\\UFCFailedData1.csv'
